...
# NOTE (verl-omni vendoring): the upstream `Qwen3TTSTokenizer` lives in
# the qwen-tts package's `inference.qwen3_tts_tokenizer` module, which is
# only needed to encode ref-audio waveforms into codec tokens at
# generation time. The training-side recipe consumes codec tokens that
# already come out of vLLM-Omni rollouts and never invokes the speech
# tokenizer, so we drop the import here and the speech-tokenizer
# side-effect inside `from_pretrained` below.
...
class Qwen3TTSForConditionalGeneration(Qwen3TTSPreTrainedModel, GenerationMixin):
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        # NOTE (verl-omni vendoring): the upstream `from_pretrained` then
        # downloads + loads the `speech_tokenizer/` subdir (an audio→codec
        # encoder used to turn ref_audio waveforms into codec tokens at
        # inference time) and a `generation_config.json`. The training
        # recipe in verl-omni never invokes either path: codec tokens for
        # actor updates come from the vLLM-Omni rollout, and the FSDP
        # actor's `compute_log_prob` calls `forward_training` directly
        # without consulting `model.generate`. We skip both side-effects
        # to keep this `from_pretrained` self-contained for training-only
        # use. If you need inference-side speech tokenization, instantiate
        # the upstream `qwen_tts` package's `Qwen3TTSTokenizer` separately
        # and assign it via `model.load_speech_tokenizer(...)`.
