...
class Qwen3TTSConfig(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # verl-omni layout bridge: upstream verl's FSDP / monkey-patch
        # paths read `config.num_attention_heads`, `config.hidden_size`,
        # etc. directly off the top-level config and `config.text_config`
        # for the VLM-style fallback. Qwen3TTSConfig keeps those fields
        # under `talker_config` only, so promote them up here and expose
        # `text_config` as a property alias to `talker_config`.
        for _bridged_field in (
            "hidden_size",
            "num_attention_heads",
            "num_key_value_heads",
            "num_hidden_layers",
            ...
            if getattr(self, _bridged_field, None) is None:
                _value = getattr(self.talker_config, _bridged_field, None)
                if _value is not None:
                    setattr(self, _bridged_field, _value)

    @property
    def text_config(self):
        """Alias for `talker_config` used by verl's VLM-style config inspection
        paths (see `verl.models.transformers.monkey_patch`)."""
